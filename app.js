// app.js — Multiplayer + Chat + Export (pure JS, no HTML)

// Firebase deps (browser ESM CDN)
import {
...
// Normalize cards into a common shape
const norm = c => ({
...
function wireRoom(){
  ...
  els.reset.onclick = async ()=>{
    ...
    await runTransaction(db, async (tx)=>{
      const s = await tx.get(roomRef);
      tx.update(roomRef, {
        deck: shuffle(baseDeck.slice().map(norm)),
        deckIndex: 0,
        lastCard: null,
